...
Hit s to start the ball spawning and data recording
    ...
    def run(self):
        ...
        while not self.done:
            ...
            if self.spawn == True:
                ...
                spawn_cmd.data = self.spawning
                self.run_spawn_publisher.publish(spawn_cmd)
                self.spawn = False # Only publish once
        ...
        while(1):
            ...
            if key in moveBindings.keys():
                ...
                if (status == 14):
                    print(msg)
                status = (status + 1) % 15
            elif key == exitBinding:
                rospy.loginfo("Saving data...")
                pub_thread.save = True
            elif key == spawnBinding:
                rospy.loginfo("Switching spawn run_mode...")
                pub_thread.spawn = True
            else:
